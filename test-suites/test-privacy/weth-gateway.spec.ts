...
makeSuite('Use native ETH at LendingPool via WETHGateway', (testEnv: TestEnv) => {
  ...
  it('Deposit WETH via WethGateway and DAI', async () => {
    ...
    const aTokensBalance = await aWETH.connect(user.signer).balanceOf(user.address);
  ...
  it('Withdraw WETH - Partial', async () => {
    ...
    const afterPartialATokensBalance = await aWETH.connect(user.signer).balanceOf(user.address);
  ...
  it('Withdraw WETH - Full', async () => {
    ...
    const afterFullATokensBalance = await aWETH.connect(user.signer).balanceOf(user.address);
    const gasCosts = approveGas.add(withdrawGas).mul(approveTx.gasPrice);
  ...
  it('Borrow stable WETH and Full Repay with ETH', async () => {
    ...
    const aTokensBalance = await aDai.connect(user.signer).balanceOf(user.address);

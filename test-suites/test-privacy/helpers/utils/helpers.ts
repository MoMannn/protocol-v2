...
export const getUserData = async (
  ...
  const [userData, scaledATokenBalance] = await Promise.all([
    helper.connect(user.signer).getUserReserveData(reserve, user.address),
    getATokenUserData(reserve, user, helper),
  ]);

  const token = await getMintableERC20(reserve);
  const walletBalance = new BigNumber((await token.connect(sender?.signer || user.signer).balanceOf(sender?.address || user.address)).toString());
...
const getATokenUserData = async (
  ...
  const scaledBalance = await aToken.connect(user.signer).scaledBalanceOf(user.address);

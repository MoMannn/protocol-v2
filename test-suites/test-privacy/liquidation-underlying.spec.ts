...
makeSuite('LendingPool liquidation - liquidator receiving the underlying asset', (testEnv) => {
  ...
  it('Deposits WETH, borrows DAI', async () => {
    ...
    const userGlobalData = await pool.getUserAccountDataPrivate(userId);
  ...
  it('Liquidates the borrow', async () => {
    ...
    const userReserveDataBefore = await helpersContract.getUserReserveDataPrivate(
      dai.address,
      userId
    );
    ...
    const tx = await pool
      ...
      .liquidationCallPrivate(weth.address, dai.address, userId, amountToLiquidate, false);

    const userReserveDataAfter = await helpersContract.getUserReserveDataPrivate(
      dai.address,
      userId
    );
    ...
    const stableDebtBeforeTx = calcExpectedStableDebtTokenBalance(
      new BigNumber(userReserveDataBefore.principalStableDebt.toString()),
      new BigNumber(userReserveDataBefore.stableBorrowRate.toString()),
      new BigNumber(userReserveDataBefore.stableRateLastUpdated.toString()),
      txTimestamp
    );
  ...
  it('User 4 deposits 10 AAVE - drops HF, liquidates the AAVE, which results on a lower amount being liquidated', async () => {
    ...
    await pool
      .connect(liquidator.signer)
      .liquidationCallPrivate(aave.address, usdc.address, userId, amountToLiquidate, false);

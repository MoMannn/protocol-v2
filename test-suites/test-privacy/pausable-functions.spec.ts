...
makeSuite('Pausable Pool', (testEnv: TestEnv) => {
  ...
  it('User 0 deposits 1000 DAI. Configurator pauses pool. Transfers to user 1 reverts. Configurator unpauses the network and next transfer succees', async () => {
    ...
    const user0Balance = await aDai.connect(users[0].signer).balanceOf(users[0].address);
    const user1Balance = await aDai.connect(users[1].signer).balanceOf(users[1].address);
    ...
    const pausedFromBalance = await aDai.connect(users[0].signer).balanceOf(users[0].address);
    const pausedToBalance = await aDai.connect(users[1].signer).balanceOf(users[1].address);
    ...
    const fromBalance = await aDai.connect(users[0].signer).balanceOf(users[0].address);
    const toBalance = await aDai.connect(users[1].signer).balanceOf(users[1].address);
  ...
  it('Liquidation call', async () => {
    ...
    const userReserveDataBefore = await helpersContract.connect(borrower.signer).getUserReserveData(
      usdc.address,
      borrower.address
    );
